...
import { ParameterSubstitutionError, substituteParameters } from "./query-params";
...
describe("substituteParameters: mssql", () => {
  ...
  it("inlines values as literals, strings as N'…', with no bind values", () => {
    ...
    expect(
      sub("SELECT {{t}}, {{big}}, {{dec}}, {{bin}}", {
        t: true,
        big: 9007199254740993n,
        dec: new SqlDecimal("-0.50"),
        bin: new Uint8Array([0, 255, 16]),
      }).sql,
    ).toBe("SELECT 1, 9007199254740993, (-0.50), 0x00ff10");
  });
...
describe("substituteParameters: duckdb", () => {
  ...
  it("inlines values as literals, with no bind values", () => {
    ...
    expect(
      sub("SELECT {{big}}, {{dec}}, {{bin}}, {{t}}", {
        big: 9007199254740993n,
        dec: new SqlDecimal("1e-3"),
        bin: new Uint8Array([0, 255]),
        t: true,
      }).sql,
    ).toBe("SELECT 9007199254740993, 0.001, from_hex('00ff'), 1");
  });
