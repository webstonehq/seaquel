...
import { countQuery, hasRowLimit, sqlTokens, stripTrailingOrderBy } from "./sql-scan";
...
describe("hasRowLimit", () => {
  ...
  it("follows each engine's string escapes", () => {
    ...
    expect(sqlTokens("SELECT x#y FROM t", "mysql").map((t) => t.text)).toEqual(["SELECT", "x"]);
    expect(sqlTokens("SELECT * FROM #tmp", "mssql").map((t) => t.text)).toEqual([
      "SELECT",
      "*",
      "FROM",
      "#tmp",
...
describe("sqlTokens", () => {
  it("tracks depth and skips comments", () => {
    const tokens = sqlTokens("SELECT (a) -- x\n FROM t", "postgres");
    expect(tokens.map((t) => [t.text, t.depth])).toEqual([
      ["SELECT", 0],
      ["(", 0],
      ["a", 1],
      [")", 0],
  ...
  it("runs an unclosed quote to the end", () => {
    expect(sqlTokens("SELECT 'abc", "mssql").map((t) => t.text)).toEqual(["SELECT", "'abc"]);
    expect(sqlTokens("SELECT [abc", "mssql").map((t) => t.text)).toEqual(["SELECT", "[abc"]);
  });
});
