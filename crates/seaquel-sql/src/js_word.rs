//! Unicode general categories as JavaScript's regexes see them, for the
//! scanner's word characters (fix 19) and the TS's old word rule, which the
//! read-only check still reads with.
//!
//! - The TS rule: one UTF-16 unit matching `[\p{L}\p{N}]`, so only BMP letters
//!   and numbers ([`is_ts_word_char`]).
//! - SQL Server (fix 19): `[\p{L}\p{N}\p{M}]` by code point
//!   ([`is_mssql_word_char`]).
//! - A `$tag$` under the TS rule: `\p{L}` by code point ([`is_letter`]).
//!
//! The tables are generated with Node, whose Unicode version is on each
//! table, by testing every code point above ASCII (surrogates left out):
//!
//! ```js
//! // ranges of consecutive code points c with re.test(String.fromCodePoint(c))
//! for (const re of [/\p{L}/u, /\p{M}/u, /\p{N}/u]) …
//! ```

fn in_table(table: &[(u32, u32)], c: char) -> bool {
    let c = u32::from(c);
    let k = table.partition_point(|&(_, hi)| hi < c);
    table.get(k).is_some_and(|&(lo, _)| lo <= c)
}
...
/// The TS word rule for a non-ASCII char: `[\p{L}\p{N}]` tested on one UTF-16
/// unit, so never a char outside the BMP (two surrogates to the TS).
pub(crate) fn is_ts_word_char(c: char) -> bool {
    u32::from(c) <= 0xFFFF && (in_table(LETTER, c) || in_table(NUMBER, c))
}

/// SQL Server's word rule for a non-ASCII char (fix 19):
/// `[\p{L}\p{N}\p{M}]`, by code point.
pub(crate) fn is_mssql_word_char(c: char) -> bool {
    in_table(LETTER, c) || in_table(NUMBER, c) || in_table(MARK, c)
}

/// `\p{L}` above ASCII: 682 ranges, from Node with Unicode 17.0.
#[rustfmt::skip]
const LETTER: &[(u32, u32)] = &[
    ...
    (0x0889, 0x088F), (0x08A0, 0x08C9), (0x0904, 0x0939), (0x093D, 0x093D),
...
/// `\p{M}` above ASCII: 327 ranges, from Node with Unicode 17.0.
#[rustfmt::skip]
const MARK: &[(u32, u32)] = &[
...
/// `\p{N}` above ASCII: 145 ranges, from Node with Unicode 17.0.
#[rustfmt::skip]
const NUMBER: &[(u32, u32)] = &[
    (0x00B2, 0x00B3), (0x00B9, 0x00B9), (0x00BC, 0x00BE), (0x0660, 0x0669),
    (0x06F0, 0x06F9), (0x07C0, 0x07C9), (0x0966, 0x096F), (0x09E6, 0x09EF),
    (0x09F4, 0x09F9), (0x0A66, 0x0A6F), (0x0AE6, 0x0AEF), (0x0B66, 0x0B6F),
    (0x0B72, 0x0B77), (0x0BE6, 0x0BF2), (0x0C66, 0x0C6F), (0x0C78, 0x0C7E),
    (0x0CE6, 0x0CEF), (0x0D58, 0x0D5E), (0x0D66, 0x0D78), (0x0DE6, 0x0DEF),
    (0x0E50, 0x0E59), (0x0ED0, 0x0ED9), (0x0F20, 0x0F33), (0x1040, 0x1049),
    ...
    (0x10D40, 0x10D49), (0x10E60, 0x10E7E), (0x10F1D, 0x10F26), (0x10F51, 0x10F54),
    ...
    (0x11730, 0x1173B), (0x118E0, 0x118F2), (0x11950, 0x11959), (0x11BF0, 0x11BF9),
    (0x11C50, 0x11C6C), (0x11D50, 0x11D59), (0x11DA0, 0x11DA9), (0x11DE0, 0x11DE9),
...
mod tests {
    ...
    #[test]
    fn categories() {
        for c in ['é', '東', 'ß', '٣', '²', 'ก', 'ᄀ', 'ſ'] {
            assert!(is_ts_word_char(c), "{c:?}");
        }
        // Marks, symbols, astral letters and emoji aren't TS word characters.
        for c in ['\u{E35}', '\u{301}', 'Ⓐ', '\u{20000}', '😀', '€', '\u{A0}'] {
            assert!(!is_ts_word_char(c), "{c:?}");
        }
        for c in ['\u{E35}', '\u{301}', '\u{20000}', '\u{1D7CE}', 'é'] {
            assert!(is_mssql_word_char(c), "{c:?}");
        }
        for c in ['Ⓐ', '😀', '€', '\u{A0}'] {
            assert!(!is_mssql_word_char(c), "{c:?}");
        }
        assert!(
            is_letter('東') && is_letter('\u{20000}') && !is_letter('٣') && !is_letter('\u{301}')
        );
    }
}
